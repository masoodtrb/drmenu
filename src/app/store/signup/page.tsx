...
export default function StoreSignupPage() {
    ...
    const signupMutation = trpc.auth.signUp.useMutation({
        onSuccess: (data) => {
            setSuccess('حساب کاربری با موفقیت ایجاد شد. لطفاً کد تایید ارسال شده را وارد کنید.');
            setError('');
            setIsLoading(false);
            // Redirect to OTP verification page
            router.push(`/store/otp?username=${encodeURIComponent(data.username)}&type=signup&token=${encodeURIComponent(data.token)}`);
        },
        onError: (error) => {
                ...
                <CardContent>
                    ...
                    <div className="mt-6 text-center space-y-4">
                        <p className="text-sm text-slate-600 dark:text-slate-400">
                            ...
                            <Link href="/store/signin" className="text-green-600 hover:text-green-700 dark:text-green-400 dark:hover:text-green-300 font-medium">
                                ورود
                            </Link>
                        </p>
