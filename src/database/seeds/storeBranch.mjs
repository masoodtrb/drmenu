...
export const storeBranchSeed = async () => {
  ...
  try {
    ...
    const storesWithoutBranches = stores.filter(
      store =>
        !storesWithBranches.some(
          storeWithBranch => storeWithBranch.id === store.id
        )
    );

    console.log(`Will create branches for ${storesWithBranches.length} stores`);
    console.log(
      `${storesWithoutBranches.length} stores will remain without branches`
    );
    ...
    for (const store of storesWithBranches) {
      ...
      for (let i = 0; i < branchCount; i++) {
        const templates =
          branchNameTemplates[store.storeType.title] ||
          branchNameTemplates['Retail Store'];
        const randomTemplate =
          templates[Math.floor(Math.random() * templates.length)];
        const randomDescription =
          branchDescriptions[
            Math.floor(Math.random() * branchDescriptions.length)
          ];
    ...
    for (let i = 0; i < branchesToCreate.length; i += batchSize) {
      ...
      console.log(
        `Created batch ${Math.floor(i / batchSize) + 1} of branches (${batch.length} branches)`
      );
    }
    ...
    console.log(
      `   - Average branches per store: ${(totalBranches / storesWithBranchesCount).toFixed(1)}`
    );
